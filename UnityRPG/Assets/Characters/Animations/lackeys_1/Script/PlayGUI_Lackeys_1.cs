...
public class PlayGUI_Lackeys_1 : MonoBehaviour {
	...
	private void initButtonInfo() {
		...
		animationButtonGroup.itemName.Add("dodge");
	...
	private void closeAnimation() {
		...
		case "GENERAL":
			for (int j = 0; j < animator.Length; j++) {
			  ...
			  animator[j].SetBool("battle_death1ToBattle_idle", true);
			}
			...
			if (stateInfo.IsName("Base Layer.general_idle0")) {
				...
				for (int j = 0; j < animator.Length; j++) {
				  ...
				  animator[j].SetBool("general_idle0ToGeneral_idle2", false);
		...
		case "BATTLE":
			...
			if (stateInfo.IsName("Base Layer.battle_idle")) {
		        for (int j = 0; j < animator.Length; j++) {
					...
					animator[j].SetBool("battle_death1ToBattle_idle", false);
				}
				...
				for (int j = 0; j < animator.Length; j++) {
		          ...
		          animator[j].SetBool("battle_idleToBattle_skill0", false);
				  animator[j].SetBool("battle_idleToBattle_skill1", false);
		          animator[j].SetBool("battle_idleToBattle_dodge", false);
				  animator[j].SetBool("battle_idleToBattle_wound0", false);
			      animator[j].SetBool("battle_idleToBattle_wound1", false);
		          animator[j].SetBool("battle_idleToBattle_death0", false);
				  animator[j].SetBool("battle_idleToBattle_death1", false);
	...
	private void setAnimation() {
		if (currentState != "") {
			...
			if (stateInfo.IsName("Base Layer.battle_death1") && currentState != "death1") {
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("battle_death1ToBattle_idle", true);
				}
			}
			...
			case "idle2":
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("general_idle0ToGeneral_idle2", true);
				}				
				break;	
	        case "walk":
				for (int j = 0; j < animator.Length; j++) {
			...
			case "skill0":
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("battle_idleToBattle_skill0", true);
				}
				break;
			case "skill1":
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("battle_idleToBattle_skill1", true);
				}
				break;	
			case "dodge":
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("battle_idleToBattle_dodge", true);
				}
				break;	
			case "wound0":
				for (int j = 0; j < animator.Length; j++) {
			...
			case "wound1":
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("battle_idleToBattle_wound1", true);
				}
				break;		
			case "death0":
				for (int j = 0; j < animator.Length; j++) {
			...
			case "death1":
				for (int j = 0; j < animator.Length; j++) {
					animator[j].SetBool("battle_idleToBattle_death1", true);
				}
				break;	
